...
public abstract class QuantizationOps
{
    ...
    [PuzzleCornerPiece(QuantizationConfig.KeyHSum256, true, null,
        "avx",    $"{MH}.{nameof(MathHelpers.HSum256_Avx)}",
        "sse3",   $"{MH}.{nameof(MathHelpers.HSum256_Sse3)}",
        "sse",    $"{MH}.{nameof(MathHelpers.HSum128_Sse)}",
        "scalar", $"{MH}.{nameof(MathHelpers.HSum256_Scalar)}")]
    public abstract float HSum256(Vector256<float> v);

    [PuzzleCornerPiece(QuantizationConfig.KeyHalfToFloat, true, null,
        "f16c",   $"{NS}.{nameof(QuantizationKernels.HalfToFloat_F16C)}",
        "scalar", $"{NS}.{nameof(QuantizationKernels.HalfToFloat_Scalar)}")]
    public abstract float HalfToFloat(ushort half);

    [PuzzleCornerPiece(QuantizationConfig.KeyGetScaleMinK4_Scale, true, null,
        "scalar", $"{NS}.{nameof(QuantizationKernels.GetScaleMinK4_Scale_Scalar)}")]
    ...
    [PuzzleCornerPiece(QuantizationConfig.KeyGetScaleMinK4_Min, true, null,
        "scalar", $"{NS}.{nameof(QuantizationKernels.GetScaleMinK4_Min_Scalar)}")]
