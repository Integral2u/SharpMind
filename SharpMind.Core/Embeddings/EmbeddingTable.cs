...
public sealed class EmbeddingTable : IDisposable
{
    ...
    public Tensor<float> Forward(ReadOnlySpan<int> tokenIds, Memory.IWorkspace? workspace = null)
    {
        ThrowIfDisposed();
    ...
    /// <param name="tokenIds">Token ID per position. Values must be in [0, VocabSize).</param>
    /// <param name="workspace">Optional workspace to rent the result tensor from.</param>
    public Tensor<float> Forward(Tensor<int> tokenIds, Memory.IWorkspace? workspace = null)
    {
        ThrowIfDisposed();
        if (tokenIds.Rank != 2)
