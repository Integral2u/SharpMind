...
/// </summary>
public readonly struct AttentionGradients(Tensor<float> dQ, Tensor<float> dK, Tensor<float> dV)
{
    public Tensor<float> DQ { get; } = dQ;
    public Tensor<float> DK { get; } = dK;
    public Tensor<float> DV { get; } = dV;
}
