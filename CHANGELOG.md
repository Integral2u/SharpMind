...
- **LFM2 support (short-conv blocks)** — validated end to end (load → prefill → decode, Q8_0, `LFM2.5-2.6B-Q8_0.gguf`) and added to the README compatibility matrix. LFM2 (Section 2.2 of its technical report) replaces the attention sublayer in most blocks with a **short-conv branch**: a single in-projection fans out to `[b, c, x]` gates, a depthwise 1-D conv mixes each channel's short history, an output gate applies `c`, and an out-projection returns to the hidden dimension. The layer's conv history lives in a per-layer `ShortConvCache` (rows = `l_cache − 1`), so a short-conv block carries no KV-cache machinery; all three generators (`StandardGenerator`, `SpeculativeGenerator`, `MedusaGenerator`) and `MedusaHeads` allocate a `ShortConvCache` for short-conv layers and a `KVCache` for the residual attention layers. The depthwise kernel is always F32 (`WScConv`, `[l_cache, hidden]`) and is loaded via the block float path, transposed from the GGUF's channel-major `{l_cache, hidden}` layout into the `[tap][channel]` order `ApplyConv` reads.
...
- **Real-model tests** — `Lfm2RealLoadTests` and `GpuQuantizedResidentTests` pointed at local GGUF files (`C:\Users\tarra\SharpMind\Models\...`). Repository tests must be synthetic-only with no external model-file dependency, so both were removed; their coverage lives on in the synthetic `Lfm2ShortConvTests` and the reference-model-driven suite.
...
- **LFM2 access violation (0xC0000005) on load** — LFM2 stores its output RMSNorm as `token_embd_norm.weight`, which matched the loose "contains `token_embd`" embedding matcher and overwrote the real Q8_0 `token_embd.weight` embedding bytes with the tiny F32 norm tensor; the logits decode then read ~1 GB from an 8 KB buffer and crashed. The embedding matcher now requires the exact `token_embd.weight` name (excluding `token_embd_norm` and `per_layer_token_embd`), and `token_embd_norm.weight` is routed to the final output norm (`FinalNormWeight`).
- **LFM2 generated only EOS (coherence collapse)** — every short-conv layer ran a zeroed depthwise kernel because `shortconv.conv.weight` was never resolved to its block: `TransformerWeights.ResolveTarget` only matched `shortconv.in_proj`/`shortconv.out_proj`, so the F32 kernel fell through, the loader's float path early-returned, and `WScConv` stayed null. It now returns `(null, block, null)` for `shortconv.conv.weight`, so the kernel loads through `ResolveFloatTarget` and the blocks actually mix history. Separately, `ShortConvLayer.Forward` now advances the cache's position length by the tokens processed (mirroring `AttentionLayer`), so a short-conv layer in slot 0 keeps the RoPE position offset correct for the residual attention layers; without this every token was processed at position 0.
