...
public sealed class InterceptingFileSystem : IFileSystem
{
    ...
    /// File-system APIs are synchronous, so we must bridge here. The check runs
    /// on a thread-pool thread so the caller's synchronization context can never
    /// be captured and deadlock the awaiting continuation.
    /// Throws <see cref="UnauthorizedAccessException"/> when access is denied.
    /// </summary>
    private void Gate(string path)
    {
        if (_check is null) return;
        bool ok = Task.Run(() => _check(_currentTool, ToolCategory.File, path, _currentArgs)
    ...
    private sealed class GatedFile(IFile inner, Action<string> gate) : IFile
    {
        ...
        public void SetAttributes(string path, FileAttributes fileAttributes) { gate(path); inner.SetAttributes(path, fileAttributes); }
        ...
        public void AppendAllBytes(string path, byte[] bytes) { gate(path); inner.AppendAllBytes(path, bytes); }

        public void AppendAllBytes(string path, ReadOnlySpan<byte> bytes) { gate(path); inner.AppendAllBytes(path, bytes); }

        public Task AppendAllBytesAsync(string path, byte[] bytes, CancellationToken cancellationToken = default)
        { gate(path); return inner.AppendAllBytesAsync(path, bytes, cancellationToken); }

        public Task AppendAllBytesAsync(string path, ReadOnlyMemory<byte> bytes, CancellationToken cancellationToken = default)
        { gate(path); return inner.AppendAllBytesAsync(path, bytes, cancellationToken); }

        public void AppendAllLines(string path, IEnumerable<string> contents) { gate(path); inner.AppendAllLines(path, contents); }

        public void AppendAllLines(string path, IEnumerable<string> contents, Encoding encoding) { gate(path); inner.AppendAllLines(path, contents, encoding); }

        public Task AppendAllLinesAsync(string path, IEnumerable<string> contents, CancellationToken cancellationToken = default)
        { gate(path); return inner.AppendAllLinesAsync(path, contents, cancellationToken); }
        ...
        { gate(path); return inner.AppendAllLinesAsync(path, contents, encoding, cancellationToken); }

        public void AppendAllText(string path, string? contents, Encoding encoding) { gate(path); inner.AppendAllText(path, contents, encoding); }

        public void AppendAllText(string path, ReadOnlySpan<char> contents) { gate(path); inner.AppendAllText(path, contents); }

        public void AppendAllText(string path, ReadOnlySpan<char> contents, Encoding encoding) { gate(path); inner.AppendAllText(path, contents, encoding); }

        public Task AppendAllTextAsync(string path, string? contents, CancellationToken cancellationToken = default)
        { gate(path); return inner.AppendAllTextAsync(path, contents, cancellationToken); }

        public Task AppendAllTextAsync(string path, string? contents, Encoding encoding, CancellationToken cancellationToken = default)
        { gate(path); return inner.AppendAllTextAsync(path, contents, encoding, cancellationToken); }
        ...
        { gate(sourceFileName); gate(destFileName); inner.Copy(sourceFileName, destFileName, overwrite); }

        FileSystemStream IFile.Create(string path) { gate(path); return inner.Create(path); }

        public FileSystemStream Create(string path, int bufferSize) { gate(path); return inner.Create(path, bufferSize); }

        public FileSystemStream Create(string path, int bufferSize, FileOptions options) { gate(path); return inner.Create(path, bufferSize, options); }

        public IFileSystemInfo CreateSymbolicLink(string path, string pathToTarget)
        { gate(path); gate(pathToTarget); return inner.CreateSymbolicLink(path, pathToTarget); }
        [SupportedOSPlatform("windows")]
        public void Decrypt(string path) { gate(path); inner.Decrypt(path); }
        [SupportedOSPlatform("windows")]
        public void Encrypt(string path) { gate(path); inner.Encrypt(path); }
        ...
        { gate(sourceFileName); gate(destFileName); inner.Move(sourceFileName, destFileName, overwrite); }

        FileSystemStream IFile.Open(string path, FileMode mode) { gate(path); return inner.Open(path, mode); }

        public FileSystemStream Open(string path, FileMode mode, FileAccess access) { gate(path); return inner.Open(path, mode, access); }

        public FileSystemStream Open(string path, FileMode mode, FileAccess access, FileShare share) { gate(path); return inner.Open(path, mode, access, share); }

        public FileSystemStream Open(string path, FileStreamOptions options) { gate(path); return inner.Open(path, options); }

        FileSystemStream IFile.OpenRead(string path) { gate(path); return inner.OpenRead(path); }

        FileSystemStream IFile.OpenWrite(string path) { gate(path); return inner.OpenWrite(path); }

        public string[] ReadAllLines(string path, Encoding encoding) { gate(path); return inner.ReadAllLines(path, encoding); }

        public Task<string[]> ReadAllLinesAsync(string path, CancellationToken cancellationToken = default)
        { gate(path); return inner.ReadAllLinesAsync(path, cancellationToken); }

        public Task<string[]> ReadAllLinesAsync(string path, Encoding encoding, CancellationToken cancellationToken = default)
        { gate(path); return inner.ReadAllLinesAsync(path, encoding, cancellationToken); }

        public Task<string> ReadAllTextAsync(string path, Encoding encoding, CancellationToken cancellationToken = default)
        { gate(path); return inner.ReadAllTextAsync(path, encoding, cancellationToken); }

        public IEnumerable<string> ReadLines(string path, Encoding encoding) { gate(path); return inner.ReadLines(path, encoding); }

        public IAsyncEnumerable<string> ReadLinesAsync(string path, CancellationToken cancellationToken = default)
        { gate(path); return inner.ReadLinesAsync(path, cancellationToken); }

        public IAsyncEnumerable<string> ReadLinesAsync(string path, Encoding encoding, CancellationToken cancellationToken = default)
        { gate(path); return inner.ReadLinesAsync(path, encoding, cancellationToken); }

        public void Replace(string sourceFileName, string destinationFileName, string? destinationBackupFileName)
        { gate(sourceFileName); gate(destinationFileName); if (destinationBackupFileName != null) gate(destinationBackupFileName); inner.Replace(sourceFileName, destinationFileName, destinationBackupFileName); }

        public void Replace(string sourceFileName, string destinationFileName, string? destinationBackupFileName, bool ignoreMetadataErrors)
        { gate(sourceFileName); gate(destinationFileName); if (destinationBackupFileName != null) gate(destinationBackupFileName); inner.Replace(sourceFileName, destinationFileName, destinationBackupFileName, ignoreMetadataErrors); }

        public IFileSystemInfo? ResolveLinkTarget(string linkPath, bool returnFinalTarget)
        { gate(linkPath); return inner.ResolveLinkTarget(linkPath, returnFinalTarget); }
        ...
        public void SetCreationTime(string path, DateTime creationTime) { gate(path); inner.SetCreationTime(path, creationTime); }
        ...
        public void SetCreationTimeUtc(string path, DateTime creationTimeUtc) { gate(path); inner.SetCreationTimeUtc(path, creationTimeUtc); }
        ...
        public void SetLastAccessTime(string path, DateTime lastAccessTime) { gate(path); inner.SetLastAccessTime(path, lastAccessTime); }
        ...
        public void SetLastAccessTimeUtc(string path, DateTime lastAccessTimeUtc) { gate(path); inner.SetLastAccessTimeUtc(path, lastAccessTimeUtc); }
        ...
        public void SetLastWriteTime(string path, DateTime lastWriteTime) { gate(path); inner.SetLastWriteTime(path, lastWriteTime); }
        ...
        public void SetLastWriteTimeUtc(string path, DateTime lastWriteTimeUtc) { gate(path); inner.SetLastWriteTimeUtc(path, lastWriteTimeUtc); }
        ...
        [UnsupportedOSPlatform("windows")]
        public void SetUnixFileMode(string path, UnixFileMode mode) { gate(path); inner.SetUnixFileMode(path, mode); }
        [UnsupportedOSPlatform("windows")]
        public void SetUnixFileMode(SafeFileHandle fileHandle, UnixFileMode mode) => inner.SetUnixFileMode(fileHandle, mode);

        public void WriteAllBytes(string path, ReadOnlySpan<byte> bytes) { gate(path); inner.WriteAllBytes(path, bytes); }

        public Task WriteAllBytesAsync(string path, ReadOnlyMemory<byte> bytes, CancellationToken cancellationToken = default)
        { gate(path); return inner.WriteAllBytesAsync(path, bytes, cancellationToken); }

        public void WriteAllLines(string path, string[] contents) { gate(path); inner.WriteAllLines(path, contents); }

        public void WriteAllLines(string path, string[] contents, Encoding encoding) { gate(path); inner.WriteAllLines(path, contents, encoding); }

        public void WriteAllLines(string path, IEnumerable<string> contents, Encoding encoding) { gate(path); inner.WriteAllLines(path, contents, encoding); }

        public Task WriteAllLinesAsync(string path, IEnumerable<string> contents, CancellationToken cancellationToken = default)
        { gate(path); return inner.WriteAllLinesAsync(path, contents, cancellationToken); }
        ...
        { gate(path); return inner.WriteAllLinesAsync(path, contents, encoding, cancellationToken); }

        public void WriteAllText(string path, ReadOnlySpan<char> contents) { gate(path); inner.WriteAllText(path, contents); }

        public void WriteAllText(string path, ReadOnlySpan<char> contents, Encoding encoding) { gate(path); inner.WriteAllText(path, contents, encoding); }

        public Task WriteAllTextAsync(string path, string? contents, Encoding encoding, CancellationToken cancellationToken = default)
        { gate(path); return inner.WriteAllTextAsync(path, contents, encoding, cancellationToken); }

        public Task WriteAllTextAsync(string path, ReadOnlyMemory<char> contents, CancellationToken cancellationToken = default)
        { gate(path); return inner.WriteAllTextAsync(path, contents, cancellationToken); }
    ...
    private sealed class GatedDirectory(IDirectory inner, Action<string> gate) : IDirectory
    {
        ...
        public void SetCurrentDirectory(string path) { gate(path); inner.SetCurrentDirectory(path); }
        ...
        public IDirectoryInfo CreateDirectory(string path, UnixFileMode unixCreateMode) { gate(path); return inner.CreateDirectory(path, unixCreateMode); }

        public IFileSystemInfo CreateSymbolicLink(string path, string pathToTarget)
        { gate(path); gate(pathToTarget); return inner.CreateSymbolicLink(path, pathToTarget); }

        public IDirectoryInfo CreateTempSubdirectory(string? prefix = null) { gate(prefix ?? System.IO.Path.GetTempPath()); return inner.CreateTempSubdirectory(prefix); }

        public IEnumerable<string> EnumerateDirectories(string path, string searchPattern) { gate(path); return inner.EnumerateDirectories(path, searchPattern); }

        public IEnumerable<string> EnumerateDirectories(string path, string searchPattern, SearchOption searchOption) { gate(path); return inner.EnumerateDirectories(path, searchPattern, searchOption); }

        public IEnumerable<string> EnumerateDirectories(string path, string searchPattern, EnumerationOptions enumerationOptions) { gate(path); return inner.EnumerateDirectories(path, searchPattern, enumerationOptions); }

        public IEnumerable<string> EnumerateFiles(string path, string searchPattern) { gate(path); return inner.EnumerateFiles(path, searchPattern); }

        public IEnumerable<string> EnumerateFiles(string path, string searchPattern, SearchOption searchOption) { gate(path); return inner.EnumerateFiles(path, searchPattern, searchOption); }

        public IEnumerable<string> EnumerateFiles(string path, string searchPattern, EnumerationOptions enumerationOptions) { gate(path); return inner.EnumerateFiles(path, searchPattern, enumerationOptions); }

        public IEnumerable<string> EnumerateFileSystemEntries(string path, string searchPattern) { gate(path); return inner.EnumerateFileSystemEntries(path, searchPattern); }

        public IEnumerable<string> EnumerateFileSystemEntries(string path, string searchPattern, SearchOption searchOption) { gate(path); return inner.EnumerateFileSystemEntries(path, searchPattern, searchOption); }

        public IEnumerable<string> EnumerateFileSystemEntries(string path, string searchPattern, EnumerationOptions enumerationOptions) { gate(path); return inner.EnumerateFileSystemEntries(path, searchPattern, enumerationOptions); }

        public DateTime GetCreationTime(string path) => inner.GetCreationTime(path);

        public DateTime GetCreationTimeUtc(string path) => inner.GetCreationTimeUtc(path);

        public string[] GetDirectories(string path, string searchPattern) { gate(path); return inner.GetDirectories(path, searchPattern); }

        public string[] GetDirectories(string path, string searchPattern, SearchOption searchOption) { gate(path); return inner.GetDirectories(path, searchPattern, searchOption); }

        public string[] GetDirectories(string path, string searchPattern, EnumerationOptions enumerationOptions) { gate(path); return inner.GetDirectories(path, searchPattern, enumerationOptions); }

        public string GetDirectoryRoot(string path) => inner.GetDirectoryRoot(path);

        public string[] GetFiles(string path, string searchPattern) { gate(path); return inner.GetFiles(path, searchPattern); }

        public string[] GetFiles(string path, string searchPattern, SearchOption searchOption) { gate(path); return inner.GetFiles(path, searchPattern, searchOption); }

        public string[] GetFiles(string path, string searchPattern, EnumerationOptions enumerationOptions) { gate(path); return inner.GetFiles(path, searchPattern, enumerationOptions); }

        public string[] GetFileSystemEntries(string path, string searchPattern) { gate(path); return inner.GetFileSystemEntries(path, searchPattern); }

        public string[] GetFileSystemEntries(string path, string searchPattern, SearchOption searchOption) { gate(path); return inner.GetFileSystemEntries(path, searchPattern, searchOption); }

        public string[] GetFileSystemEntries(string path, string searchPattern, EnumerationOptions enumerationOptions) { gate(path); return inner.GetFileSystemEntries(path, searchPattern, enumerationOptions); }
