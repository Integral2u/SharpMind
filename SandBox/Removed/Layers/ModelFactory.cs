...
public static class ModelFactory
{
        ...
        bool useHooks = false)
    {
        ...
        var layerFactory = Assembler.CreateInstance<LinearLayerFactory>(cfg);
        ...
        var attn = Activator.CreateInstance(t, weights.Config, qOps, layerFactory) as AttentionLayer;
        ArgumentNullException.ThrowIfNull(attn);
        attn.SetWeights(blockWeights);
        
        var ffn = BuildFfn(weights, sharpConfig, acts, ops, qOps, layerFactory);
    ...
    private static FfnLayer BuildFfn(
        ...
        TensorOps ops,
        QuantizationOps qOps,
        LinearLayerFactory layerFactory)
    {
        var blockWeights = weights.Blocks[0]; // Weight shapes are same across blocks for FFN
        return sharpConfig.Ffn switch
        {
            FfnKind.Dense => new DenseFfnLayer(weights.Config, acts, ops, qOps, layerFactory, blockWeights),
            FfnKind.Gated => new GatedFfnLayer(weights.Config, acts, ops, qOps, layerFactory, blockWeights),
            FfnKind.MoE => new MoEFfnLayer(weights.Config, acts, ops, qOps, layerFactory, blockWeights),
