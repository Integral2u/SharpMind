...
public sealed class DenseFfnLayer(ModelConfig config, ActivationOps acts, TensorOps ops, QuantizationOps qOps, LinearLayerFactory layerFactory, TransformerWeights.BlockWeights? weights = null) : FfnLayer(config, acts, ops, FfnKind.Dense, qOps, layerFactory, weights)
{
    ...
    {
        if (W1 is not FloatLinearLayer f1 || W2 is not FloatLinearLayer f2)
        ...
        using var w2T = TensorOps.Transpose(f2.Weight);
        var dHidden = Ops.MatMul(gradOutput, w2T);
        using var w1T = TensorOps.Transpose(f1.Weight);
